...
- Inside the same Shorts session, Up follows a directional back stack and Down can return through its forward stack for **10 seconds**. Expired or unavailable entries are skipped, the history resets when the Shorts overlay closes, and replayed history items remain hidden from normal feeds.
